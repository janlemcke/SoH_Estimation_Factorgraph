...
function normalize_data!(data, norms)
    ...
    for i in eachindex(data)
        for j in 1:cols
            data[i][j] = (data[i][j] - norms[j][:mean]) / sqrt(norms[j][:var])
        end
    end
    return data
end
...
function loss_function(m, x, y, loss_choice)
    ...
    elseif loss_choice == "mre" 
        return Utils.mean_relative_error(y, m(x))
    end
end

# Inner loop for hyperparameter tuning using k_inner-fold cross-validation
function inner_cv_train(train_samples, train_targets, k_inner, hyperparams_grid, loss_choice, output_index, patience=50, epochs=10000)
    best_val_score = Inf
    best_model = nothing
    ...
    for hyperparams in hyperparams_grid
        ...
        for fold_idx in 1:k_inner
            ...
            for epoch in 1:div(epochs, 3)
                ...
                if (best_fold_val_score - val_loss) > 1e-4
                    ...
                    current_patience = 0
                    println("Epoch $(epoch) - Validation Loss: $(best_fold_val_score)")
                else
                    current_patience += 1
                end
            end
        ...
        if avg_val_score < best_val_score
            ...
            println("Best model updated with hyperparameters: $(best_hyperparams) - Average Validation Loss: $(best_val_score) - RMSE: $(sqrt(best_val_score))")
        end
    end
...
function train_approximate_gaussian_mean_factor(; variable=X, k_outer=5, k_inner=3, hyperparams_grid, seed=nothing, datapath="data/dataset_gaussian_mean_factor_20_000.jld2", datakey="data", patience=50, epochs=10000, loss_choice="kl", output_index=0)
    ...
    for fold_idx in 1:k_outer
        ...
        println("Outer fold $(fold_idx) - Test Loss: $(test_loss) with Hyperparams: $(best_inner_hyperparams)")
    ...
    for epoch in 1:epochs
        ...
        if val_loss < best_final_score
            best_final_score = val_loss
            println("Epoch $(epoch) - Final Loss: $(best_final_score)")
            current_patience = 0
        else
            current_patience += 1
        end
